...
@Order(-1)
@Component
public class ZuulExHandler implements HandlerExceptionResolver {

	@Value("${debug}")
	private boolean debug;

	public ZuulExHandler() {
	}

	private static final Logger LOG = LoggerFactory.getLogger(ZuulExHandler.class);
	...
	public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
		...
		if (null != s) {
			...
			if (null != ex && ex instanceof HttpRequestMethodNotSupportedException) {
				LOG.error(sb.toString());
			} else {
				if (debug) {
					LOG.error(sb.toString(), ex);
